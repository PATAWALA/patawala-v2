        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="w-full flex justify-center mb-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/10 rounded-full border border-blue-500/20 backdrop-blur-sm">
              ...
              <span className={`text-sm font-bold text-blue-400 tracking-tight transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
          ...
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-4 text-white tracking-tight">
              ...
              <span className={`block text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400 mt-2 font-black tracking-tight transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
                {t('titleHighlight', 'realisations')}
              </span>
            </h2>
            <p className={`text-lg text-gray-200 max-w-2xl mx-auto font-medium transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 mb-5">
                      {translated.tags?.slice(0, 3).map((tag: string) => (
                        <span
                          key={tag}
                          className={`px-2.5 py-1 bg-blue-500/10 text-blue-400 text-xs font-semibold rounded-full tracking-tight transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
                        >
                          {tag}
                        </span>
                      ))}
                      {translated.tags && translated.tags.length > 3 && (
                        <span className={`px-2.5 py-1 bg-[#0A0F1C] text-gray-300 text-xs font-medium rounded-full border border-[#1F2937] transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
          ...
          <div className="flex justify-center mt-16">
            <div className="flex flex-wrap justify-center gap-6 px-0 py-0 sm:px-8 sm:py-5 sm:bg-[#141B2B] sm:rounded-2xl sm:border sm:border-[#1F2937]">
              <div className="flex flex-col items-center">
                ...
                <span className={`text-xs text-gray-300 font-medium transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
